...
func makeContractAddresses(n int) [][]byte {
	var addrs [][]byte
	for i := 0; i < n; i++ {
		a, err := NewContractAddress([]byte(fmt.Sprintf("Contract ADDRESS %d", i)))
		if err != nil {
			panic(err) // ok
		}
...
func BenchmarkParseContractAddress(b *testing.B) {
	...
	b.Run("contract", benchTestWithAddrs(makeContractAddresses(20)))
